...
def main() -> None:
    ...
    if not args.skip_tests:
        test_result = run_command(root, [sys.executable, "-m", "pytest", "-q", "backend/tests", "-W", "error::ResourceWarning"], timeout=180)
        add_check(checks, "backend_unit_tests", test_result["ok"], "Backend tests pass under pytest with ResourceWarning treated as an error.", test_result)
