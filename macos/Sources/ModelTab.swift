...
struct HomeHeroSection: View {
    ...
    private var title: String {
        ...
        if state.connectedAIIntegrationCount > 0 {
            return "Connect notes or save from MCP"
        }
    ...
    private var detail: String {
        ...
        if state.connectedAIIntegrationCount > 0 {
            return "Use the MCP save tool from your AI app, or connect notes so Ask has something to cite."
        }
        return "Connect an AI tool through MCP or choose notes once. Cortex brings new memory to Review before Ask can use it."
    }
    ...
    private var actionTitle: String {
        ...
        if activeSources == 0 { return hasEmptySource ? "Choose notes" : "Connect MCP or notes" }
    ...
    private var actionDetail: String {
        ...
        if activeSources == 0 {
            ...
            if state.connectedAIIntegrationCount > 0 {
                return "Use MCP save or sync notes."
            }
            return "Open Connections to enable MCP or notes."
        }
        if pendingCount > 0 {
    ...
    private var sourceStatus: (detail: String, systemImage: String, color: Color) {
        ...
        return ("MCP or notes not connected", "link.badge.plus", .accentColor)
    }
    ...
    private func runNextAction() {
        if !state.isLocalServiceReady {
            ...
            if canSyncSource, let connector = obsidianConnector {
                ...
                state.openConnectionsPrivacy(statusMessage: "Connection status")
            }
        }
    }
}
