...
""",
    ...
    def test_memory_quality_report_tracks_citations_review_and_layers(self) -> None:
        ...
        self.assertIn("provenance_coverage", report)
        self.assertIn("sector_coverage", report)
        self.assertIn("source_type_coverage", report)
        self.assertIn("vector_health", report)
        self.assertIn("relation_health", report)
        self.assertLess(report["provenance_coverage"], 1.0)
        self.assertEqual(report["source_type_coverage"], 1.0)
        self.assertEqual(report["relation_health"]["status"], "ok")
        self.assertIn(report["vector_health"]["status"], {"ok", "catching_up", "not_available"})
        self.assertEqual(report["totals"]["weak_provenance_memories"], report["totals"]["active_memories"] - report["totals"]["provenance_complete_memories"])
        self.assertTrue(report["layers_present"])
        self.assertTrue(any("citations" in warning for warning in report["warnings"]))
        self.assertTrue(any("provenance" in warning for warning in report["warnings"]))
        ...
        self.assertIn("provenance_coverage", unit_health)
        self.assertIn("sector_coverage", unit_health)
        self.assertIn("source_type_coverage", unit_health)
    ...
    def test_sanitized_support_bundle_omits_user_content(self) -> None:
        ...
        self.assertIn("source_readiness", bundle)
        self.assertIn("summary", bundle["source_readiness"])
        self.assertIn("sources", bundle["source_readiness"])
        self.assertGreaterEqual(bundle["source_readiness"]["summary"]["sources_total"], 1)
