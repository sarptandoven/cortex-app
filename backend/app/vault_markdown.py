...
# ---- N3: vault Home / Map-of-Content root page --------------------------------------------------

HOME_FRONTMATTER_FIELDS: tuple[str, ...] = (
    "cortex_generated",
    "title",
    "entity_count",
    "community_count",
...
def render_home_markdown(page: dict[str, Any]) -> str:
    """Render the vault Home / "Start Here" root page. Machine-owned, deterministic (no timestamp),
    all links are [[wikilinks]] that resolve inside the vault (entity MOC stems + memory notes) or
    plain folder links to the People/Projects/Orgs/Topics folders."""
    lines: list[str] = [_FENCE]
    for field in HOME_FRONTMATTER_FIELDS:
        if field in page and page[field] is not None:
            lines.append(f"{field}: {_dump_value(page[field])}")
    lines.append(_FENCE)
    lines.append("")
    lines.append("# " + str(page.get("title") or f"{APP_BRAND} — Start Here"))
    lines.append("")
    lines.append(f"_Generated by {APP_BRAND}. Edits here are overwritten; edit your notes in `memories/`._")
    lines.append("")

    folders = page.get("folders") or []
    if folders:
        ...
        for folder in folders:
            ...
            suffix = f" ({int(count)})" if isinstance(count, int) and count > 0 else ""
            lines.append(f"- [{name}]({name}/){suffix}")
        lines.append("")

    hubs = page.get("hubs") or []
    if hubs:
        ...
        for hub in hubs:
            if not isinstance(hub, dict):
                continue
            link = _moc_alias_link(hub.get("wikilink") or "", hub.get("name") or "")
            if not link:
                continue
            kind = str(hub.get("kind") or "").strip()
            lines.append(f"- {link}" + (f" — {kind}" if kind else ""))
        lines.append("")

    areas = page.get("areas") or []
    if areas:
        lines.append("## Life areas")
        lines.append("")
        for area in areas:
            ...
            if not label:
                continue
            members = [
                _moc_alias_link(m.get("wikilink") or "", m.get("name") or "")
                for m in (area.get("members") or [])
                if isinstance(m, dict) and m.get("wikilink")
            ]
            members = [m for m in members if m]
            head = _moc_alias_link(area.get("wikilink") or "", label) if area.get("wikilink") else f"**{label}**"
            lines.append(f"- {head}" + (f": {', '.join(members)}" if members else ""))
        lines.append("")
    ...
    if recent:
        ...
        for item in recent:
            if not isinstance(item, dict):
                continue
            target = str(item.get("wikilink") or "").strip()
            note = str(item.get("note") or "").strip().replace("\n", " ")
            if target:
                lines.append(f"- {_moc_alias_link(target, note or target)}")
            elif note:
                lines.append(f"- {note}")
        lines.append("")

    open_loops = page.get("open_loops") or []
    if open_loops:
        lines.append("## Open loops")
        lines.append("")
        for loop in open_loops:
            text = str((loop.get("text") if isinstance(loop, dict) else loop) or "").strip().replace("\n", " ")
            if text:
                lines.append(f"- [ ] {text}")
        lines.append("")
...
# ---- N4: daily notes (Journal/YYYY-MM-DD.md) ----------------------------------------------------

DAILY_FRONTMATTER_FIELDS: tuple[str, ...] = (
    "cortex_generated",
    "date",
    "captures",
    "memories",
...
def render_daily_markdown(page: dict[str, Any]) -> str:
    """Render one day's Journal page: cortex_generated frontmatter + a body of [[wikilinks]] to that
    day's learned memory notes and touched entities. Machine-owned; deterministic (no timestamp)."""
    lines: list[str] = [_FENCE]
    for field in DAILY_FRONTMATTER_FIELDS:
        if field in page and page[field] is not None:
            lines.append(f"{field}: {_dump_value(page[field])}")
    lines.append(_FENCE)
    lines.append("")
    lines.append(f"# {page.get('date') or 'Journal'}")
    lines.append("")

    memory_items = page.get("memory_items") or []
    if memory_items:
        ...
        for mem in memory_items:
            if not isinstance(mem, dict):
                continue
            target = str(mem.get("wikilink") or "").strip()  # a memory note stem
            if not target:
                continue
            note = str(mem.get("note") or "").strip().replace("\n", " ")
            lines.append(f"- {_moc_alias_link(target, note or target)}")
    ...
    entity_items = page.get("entity_items") or []
    if entity_items:
        ...
        for ent in entity_items:
            if not isinstance(ent, dict):
                continue
            wikilink = str(ent.get("wikilink") or "").strip()
            name = str(ent.get("name") or "").strip()
            if wikilink:
                chips.append(_moc_alias_link(wikilink, name))
            elif name:
                chips.append(_wikilink(name))
