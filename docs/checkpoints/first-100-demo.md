...
`first100_live_smoke.py` reads the app token from the macOS Keychain when `--token` is omitted, then falls back to the legacy `~/Library/Application Support/Cortex/credentials.json` file and defaults for older local builds. It writes only under an isolated smoke user and deletes that smoke user by default.
