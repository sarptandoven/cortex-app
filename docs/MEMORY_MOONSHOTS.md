...
**Phase 1 implemented (July 2026).** `export_memory_bundle` now emits a portable-memory v1 envelope with a stable vault-local Ed25519 identity, public-key fingerprint, signed canonical payload, and independently foldable event-fingerprint chain proof. `verify_memory_bundle` supports signer pinning. `import_memory_bundle` and `POST /v1/import/bundle` verify before writing, deterministically rebind ids to the target tenant, preserve portable provenance lineage and belief snapshots, reject broken supersession, downgrade unpinned remote authorship, preserve pinned trust, and make exact replay idempotent. FastAPI, standalone, and MCP surfaces have parity. The remaining phases are the published vendor-neutral specification and the OpenClaw reference adapter.
