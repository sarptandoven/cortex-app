...
- `sync_devices` and `GET /v1/sync/changes?device_id=...` now expose a content-free, cursorable local event feed with safe metadata, object ID redaction, counts, high watermark, shard assignment, device heartbeat cursors, revocation state, and optional HMAC signatures. It is the local manifest shape for future sync uploads, not a hosted sync service by itself.
