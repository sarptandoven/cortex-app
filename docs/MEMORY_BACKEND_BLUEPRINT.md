...
- Scoped `cxa_` REST tokens can now resolve a user before shard routing, enforce read/write/export/maintenance/destructive capability classes, support metadata listing and revocation, and `CORTEX_REQUIRE_SCOPED_API_TOKENS=1` disables global-token user switching for hosted-style runs.
...
- `sync_devices`, `sync_receipts`, and `GET /v1/sync/changes?device_id=...` now expose a content-free, cursorable local event feed with safe metadata, object ID redaction, counts, high watermark, shard assignment, device heartbeat cursors, manifest acknowledgement state, revocation state, and optional HMAC signatures. It is the local manifest shape for future sync uploads, not a hosted sync service by itself.
- Hosted sync should send authenticated, signed append-only events and encrypted payload manifests, not opaque app state.
