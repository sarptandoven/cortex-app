...
- `GET /v1/sync/changes` now exposes a content-free, cursorable local event feed with safe metadata, object ID redaction, counts, high watermark, and shard assignment. It is the local manifest shape for future sync uploads, not a hosted sync service by itself.
- Hosted sync should send signed append-only events and encrypted payload manifests, not opaque app state.
