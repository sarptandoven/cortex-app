...
2. **Point DNS** — add one A record: `api.<your-domain>` → the VM IP.
...
3. **From this repo on your Mac, run one command:**
   ...
   ```bash
   deploy/push.sh root@<VM-IP> api.<your-domain>
   ```
...
4. **Verify:** `curl https://api.<your-domain>/health` → `{"status":"ok",...}` and open
...
- Add a free UptimeRobot monitor on `https://api.<domain>/ready` (5 minutes, your phone
